# Sigma Rule DSL — Specification Snapshot (`dsl_v2.0.md`)

Document ID: `dsl_v2.0`  
Status: Implemented snapshot (v1.0 stable equity product + v1.1 option context + v1.2 chain context + v2.0-A intent resolution)  
...
This document is a **full copy-forward** of `docs/dsl_v1.2.md` with **only the referenced v2.0 sections updated**
to reflect implemented v2.0-A intent-resolution semantics:
- deterministic idempotency collapse for intent emissions (v2.0-A)
- deterministic conflict resolution for multiple intents at the same event (v2.0-A)
- stable intent-decision schema additions (`is_effective`, `overridden_by`) (v2.0-A)

Updated sections in this snapshot:
- Chapter 15.4.1 (runner decision schema: intent resolution fields)
- Chapter 15.5 (idempotency + conflict resolution policy for intent decisions)
...
Implemented `chain` context fields (v1.2-A; quality-only):
- timestamp identity:
  - `chain.as_of: Timestamp` (preferred)
  - `chain.time: Timestamp` (alias)
- quality predicates:
  - `chain.is_fresh: Bool`
...
Runner note (v1.2-A, implemented):
...
- per-event evaluation records (ordered by event index), including:
  ...
  - per-rule evaluation records (ordered by deterministic rule order):
    - rule name + context,
    - branch predicate outcomes (`true`/`false`; a limited `unknown` exists as of v1.2-A for chain snapshot quality),
...
v1.2-A note (implemented, narrow scope):
- when evaluating `chain` context rules on incomplete chain snapshots (`chain.has_unknowns=true`),
  predicates that do not reference chain quality fields are treated as **Unknown** and do not match.
- `else` branches are disabled while unknowns are present (fail closed).
...
- `PlanDecision`: broker-agnostic plan outputs (implemented as Plan IR generation in v2.0-B).
...
Plan IR is generated by a post-processing CLI command:

```bash
sigmadsl run --profile intent ... > decisions.jsonl
...
**Status: CURRENTLY DECIDED (v1.0-A + v2.0-A extensions)**  
Rationale: v1.0 establishes a stable decision contract across `run`/`replay`/`diff`; v2.0-A extends intent semantics without breaking the envelope.  
...
- for `annotate` decisions:
  - `note` (string)
- for `declare_intent` / `cancel_intent` decisions:
  - `intent_action` (`declare` or `cancel`)
  - `intent_kind` (string or null)
  - `quantity` (decimal string or null)
  - `percent` (decimal string or null)
  - `reason` (string or null)
  - `is_effective` (bool) (v2.0-A)
  - `overridden_by` (decision id string or null) (v2.0-A)
...
**Status: CURRENTLY DECIDED (intent decisions only; v2.0-A)**  
Rationale: Intent decisions must be deterministic and replay-safe before any plan IR exists.  
Implications: The engine defines an explicit intent-resolution phase per event; other verb categories remain unchanged.

### 15.5.1 Intent resolution (v2.0-A)

For each `event_index`, the engine evaluates rules to emit **raw** intent decisions, then applies:

1) **Idempotency collapse**
- Idempotency key:
  - `(symbol, intent_kind, intent_action, quantity, percent)`
- If multiple intents share the same key:
...
- Conflicts are resolved deterministically with the following priority:
...
Winners remain `is_effective=true`.
Losers are marked overridden and are ignored by downstream phases.

### 15.5.2 Risk compatibility (v2.0-A)

Risk enforcement (v1.0-B) remains fail-closed, but is applied only to **effective** intents.
