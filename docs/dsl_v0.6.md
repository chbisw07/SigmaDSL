# Sigma Rule DSL — Specification Snapshot (`dsl_v0.6.md`)

Document ID: `dsl_v0.6`  
Status: Implemented snapshot (v0.6-A: imports + module layout)  
...
This document is a **full copy-forward** of `docs/dsl_v0.5.md` with **only the referenced v0.6 chapters updated**
to reflect implemented Sprint 0.6-A deterministic imports and module layout rules.
...
### 6.3.1 v0.5-A run log schema (self-contained snapshot + indicator pins)

In v0.5-A, the runner produces a **single JSON run log** with:
...
- Backward compatibility:
  - v0.5-A loaders should continue accepting `schema_version: 0.4-a` logs (which omit `indicators`).
